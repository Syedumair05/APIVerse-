...
export class CountryController {
  static getCountries = async (req: Request, res: Response) => {
    ...
    const { data, pagination, lastUpdated, cacheMeta } = await CountryService.getAllCountries(queryParams);
    if (cacheMeta) {
      res.setHeader('X-Cache-Mode', cacheMeta.cacheMode);
      res.setHeader('X-Cache-Status', cacheMeta.status);
      res.setHeader('X-Cache-Store', cacheMeta.store);
      res.setHeader('X-External-Api-Call', String(cacheMeta.externalApiCalled));
      res.setHeader('X-External-Api-Status', cacheMeta.externalApiStatus);
      res.setHeader('X-Offline-Fallback', String(cacheMeta.offlineFallbackUsed));
    }
